...
def calc_texture_gram(net):
    ...
    for layer, weight in zip(layers, style_layer_weights):
        ...
        G = tf.matmul(tf.transpose(F, perm=[0, 2, 1]), F)*weight  # [img_num, M, N]
        _, y, x = G.get_shape()
...
def calc_texture_logits(net, vgg_layers):
    ...
    fc1 = tf.layers.dense(inputs=gram, units=units, name='fc_g1', activation=None,trainable=False)
    return fc1

def calc_label_gram(net, vgg_layers):
    sess = tf.Session()
    with tf.variable_scope('fc_g1', reuse=True):
         w = tf.get_variable('kernel')
         b = tf.get_variable('bias')
    object_output = tf.one_hot(net['labels'],units)
    sess.run(tf.global_variables_initializer())
    Inverse_matrix1 = tfp.math.pinv(w)
    Inverse_matrix_array1 = sess.run(Inverse_matrix1)
    fully_bias = sess.run(b)
    fc1_b = tf.layers.dense(inputs=object_output,
                      ...
                      bias_initializer=tf.constant_initializer(-fully_bias),
                      trainable=False,
                      activation=None)
    width, _ = w.get_shape()
    fc1_a = tf.layers.dense(inputs=fc1_b,
                  units=width, # units or depth
...
def calc_obj_logits(net):
    ...
    net['o_fc2'] = fc_layer('fc_o2', tf.layers.dropout(net['o_fc1'], 0), 64, tf.nn.relu)
...
#    net['bn5_4'] = tf.layers.batch_normalization(net['conv5_4'], training=False, momentum=0.9)
    ...
    loss = texture_loss*10000 + obj_loss*3
...
def main():
    ...
    optimizer = tf.train.AdamOptimizer(learning_rate=0.00001, epsilon=1e-07)
